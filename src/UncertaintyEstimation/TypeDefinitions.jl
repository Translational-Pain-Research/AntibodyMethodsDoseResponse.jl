...
* `levels`: List of uncertainty levels. They can be probability masses of symmetric intervals around the median or fractions of the best objective value, depending on the construction.
...
Estimate uncertainty by shifting all grid weights uniformly, one bin at a time, while keeping the other bins fixed. Admissible weights (for a given level) are determined by calculating the objective value (objective function automatically generated) for the shifted weights.
...
* `levels`: List of uncertainty levels. Depending on the construction, they should correspond to the uncertainty levels of a EpitopeUncertainty object.
